...
STRATEGY = "\n".join(f"  <b>Month {m}</b> — {t}" for m, t in MONTH_TITLES.items())

WELCOME = (
    "🎯 <b>Welcome, {name}!</b>\n\n"
    "As a final-year CSE engineer, you already have the fundamentals — now we make them "
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await send_html(
        ...
        WELCOME.format(name=esc(user.first_name or "engineer"), strategy=STRATEGY),
    )
